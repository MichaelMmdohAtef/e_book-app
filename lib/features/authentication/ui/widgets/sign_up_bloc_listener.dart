  ...
  Widget build(BuildContext context) {
    return BlocListener<AuthenticationCubit, AuthenticationStates>(
      listener: (context, state) {
        if (state is OnLoadingRegisterStates) {
          showDialog(
            context: context,
        ...
        } else if (state is OnSuccessRegisterStates) {
          // showSuccessDialog(context);
        } else if (state is OnErrorRegisterStates) {
          setupErrorState(context, state.errorMessage);
        } else if (state is OnSuccessPostUserDataToFireStoreStates) {
          showSuccessDialog(context);
        } else if (state is OnErrorPostUserDataToFireStoreStates) {
          setupErrorState(context, state.errorMessage);
        }
      },
